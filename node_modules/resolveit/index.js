...
function buildPaths (search, opts) {
    ...
    filter(Boolean).
    map(function (path) { return Path.normalize(path); }).
    filter(function (path) { return path && CACHE[path] !== false; });
}

function packageReader (filepath) {
    return Path.join(
        Path.dirname(filepath),
        JSON.parse(FS.readFileSync(filepath, 'utf8')).main
    );
}
//---------------------------------------------------------------------------
// Main
...
module.exports = Object.defineProperties({
    ...
    sync: function (search, basedir, opts) {
        var findAll, allowDirs, paths, foundpath, packages;
        ...
        if (packages) {
            packages = !isArray(opts.packages) ? packages :
        ...
        paths = buildPaths(search, opts)[findAll ? 'filter' : 'some'](
            function (filepath) {
                ...
                if (cached) {
                    ...
                    // update the cache with useful information
                    CACHE[path] = exists ?
                ...
                if (exists && matches) {
                    if (packages && packages[name]) {
                        pkgReader = typeof packages[name] === 'function' ?
                            packages[name] : packageReader;
                        pkgPath = pkgReader(path);
                        foundpath = pkgPath;
                        result = !!pkgPath;
                    }
                    else {
     ...
     * @param {String} startPath optional
     * @returns {String}
     */
    directoryFromStack: { value: directoryFromStack },
    fileFromStack: { value: fileFromStack }
});
